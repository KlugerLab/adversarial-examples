...
def softthresh(attack_dir, denoise_base_dir, mul_thresh, wavelet_func):
    ...
    for l2_dir in l2_dir_list:
        ...
        output_dir  =  denoise_base_dir + softthresh.__name__ + '_' + str(mul_thresh) + '_' + wavelet_func  + '/' + attack_name + '/' + l2_distance + '/'
        ...
        script_name = 'denoise_wrapper'  #'denoise_softthresh' # if args exits: 'test Arg1 Arg2'
        cmd_name = script_name + ' ' + input_dir_adv + '/ ' + output_dir + ' ' + softthresh.__name__  + ' ' + mul_thresh + ' ' + wavelet_func
        cmd = "matlab -nodisplay -r '" + cmd_name + "; exit;'"
        subprocess.call(cmd, shell=True)
        #print('eps=' + str(eps_list[i]) + ' is done.')
